...
/* Batched column norms - compute norms of k columns of V (n x k matrix) */
void compute_col_norms_batched(int n, int k, const real_type *V, real_type *norms);

/* Batched nrm2 - compute norms of k vectors of length n stored contiguously */
void nrm2_batched(int n, int k, const real_type *V, real_type *norms);

/* Cholesky QR orthonormalization - batched with O(1) syncs */
/* Returns 0 on success, -1 if fallback to CGS2 is needed */
