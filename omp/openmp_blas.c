...
{ 
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(tofrom:v[0:n])
#else
...
void openmp_axpy (const int n, const real_type alpha, const real_type *x, real_type *y){
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(to:x[0:n]) map(tofrom:y[0:n])
#else
#pragma omp parallel for simd
#endif
  for (i = 0; i < n; ++i) {
...
#pragma omp target teams distribute parallel for \
    map(to:a[0:nnz], x[0:n], ia[0:n+1], ja[0:nnz]) map(tofrom:result[0:n]) \
    private(s, j, col)
  for (i = 0; i < n; ++i) {
    s = result[i] * beta;
...
#else
#pragma omp parallel for private(s, j, col) schedule(static)
  for (i = 0; i < n; ++i) {
    s = result[i] * beta;
    for (j = ia[i]; j < ia[i + 1]; j++) {
      col = ja[j];
...
void openmp_vec_vec(const int n, const real_type *x, const real_type *y, real_type *res)
{
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(to:x[0:n], y[0:n]) map(from:res[0:n])
#else
#pragma omp parallel for simd
#endif
  for (i = 0; i < n; ++i) {
...
void openmp_vector_reciprocal(const int n, const real_type *v, real_type *res)
{
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(to:v[0:n]) map(from:res[0:n])
#else
#pragma omp parallel for simd
#endif
  for (i = 0; i < n; ++i) {
...
void openmp_vec_copy(const int n, const real_type *src, real_type *dest)
{
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(to:src[0:n]) map(from:dest[0:n])
#else
#pragma omp parallel for simd
#endif
  for (i = 0; i < n; ++i) {
...
void openmp_vec_zero(const int n, real_type *vec)
{
  int i;
#if OMP_OFFLOAD
#pragma omp target teams distribute parallel for simd map(tofrom:vec[0:n])
#else
#pragma omp parallel for simd
#endif
  for (i = 0; i < n; ++i) {
