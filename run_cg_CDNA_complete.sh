...
SCRIPT="/home/kswirydo/LAP_PNNL/lap_hip_cg " ## This is were the command go
...
ARGX+=(["${MATRIX[6]}_${PRECOND[0]}"]='2 2') 
ARGX+=(["${MATRIX[6]}_${PRECOND[1]}"]='3 0') 
ARGX+=(["${MATRIX[6]}_${PRECOND[2]}"]='1 2') 
ARGX+=(["${MATRIX[6]}_${PRECOND[3]}"]='3 3') 
ARGX+=(["${MATRIX[6]}_${PRECOND[4]}"]='-1') 
ARGX+=(["${MATRIX[6]}_${PRECOND[5]}"]='1 1') 
...
for idx in {1..5}; do
  for precond in "${PRECOND[@]}"; do
	...
	continue; fi
      ...
      sleep 1s
      mtxs=`dirname $mtx`
      dir_name="REP_${idx}_${GPU_NAME}_CG/${mtxs}/"
      mkdir -p ${dir_name}
     ...
     echo "Entering directory: " $dir_name
      ...
      sleep 1s
echo "this is RES HOME"
      ls ${RES_HOME}
      power_sids=()
      for ((i = 0; i < 1 ; i++)); do
	echo "Will be putting results in "${RES_HOME}/${precond}.txt
        /home/kswirydo/power_analysis/power_profiler/power_profiler  ${RES_HOME}/${precond}.txt 1200 20 &
	power_sids+=($!)
      done
