...
#SBATCH --exclusive
#SBATCH --output=mi250_Laplacian.txt
#SBATCH --nodelist=TheraC67
...
SCRIPT="/home/kswirydo/LAP_PNNL/lap_hip_MI250_laplacian " ## This is were the command go
...
ARGX+=(["${MATRIX[1]}_${PRECOND[2]}"]='-1')
ARGX+=(["${MATRIX[1]}_${PRECOND[3]}"]='-1')
ARGX+=(["${MATRIX[1]}_${PRECOND[4]}"]='-1')
ARGX+=(["${MATRIX[1]}_${PRECOND[5]}"]='-1')
#hugebubbles
ARGX+=(["${MATRIX[2]}_${PRECOND[0]}"]='-1')
ARGX+=(["${MATRIX[2]}_${PRECOND[1]}"]='-1')
ARGX+=(["${MATRIX[2]}_${PRECOND[2]}"]='-1')
...
ARGX+=(["${MATRIX[3]}_${PRECOND[2]}"]='-1')
ARGX+=(["${MATRIX[3]}_${PRECOND[3]}"]='-1')
ARGX+=(["${MATRIX[3]}_${PRECOND[4]}"]='-1')
...
ARGX+=(["${MATRIX[4]}_${PRECOND[2]}"]='-1')
ARGX+=(["${MATRIX[4]}_${PRECOND[3]}"]='-1')
...
ARGX+=(["${MATRIX[6]}_${PRECOND[1]}"]='-1')
ARGX+=(["${MATRIX[6]}_${PRECOND[2]}"]='-1')
...
for idx in {1..5}; do
 ...
 #     rm -rf ${RES_OUT_HOME}/*
	...
	echo "Will be putting results in "${RES_HOME}/${precond}.txt
        /home/kswirydo/power_analysis/power_profiler/power_profiler  ${RES_HOME}/${precond}.txt 600 20 &
	power_sids+=($!)
      done
